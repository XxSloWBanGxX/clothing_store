...
$selectedOldSizes = $data['old']['sizes'] ?? [];
if (!is_array($selectedOldSizes)) $selectedOldSizes = [];
$selectedOldColors = $data['old']['colors'] ?? [];
if (!is_array($selectedOldColors)) $selectedOldColors = [];
?>
...
<section class="admin-panel-box">
    ...
    <form action="index.php?url=admin-store" method="POST" enctype="multipart/form-data" class="admin-form">
        <div class="admin-form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" id="slug" name="slug" value="<?= htmlspecialchars($data['old']['slug'] ?? ''); ?>">
            ...
            <div class="form-group">
                ...
                <select id="category_id" name="category_id">
                    ...
                    <?php foreach ($data['categories'] as $category): ?>
                        <option value="<?= (int)$category['id']; ?>" <?= (($data['old']['category_id'] ?? '') == $category['id']) ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" id="price" name="price" value="<?= htmlspecialchars($data['old']['price'] ?? ''); ?>">
            ...
            <div class="form-group">
                ...
                <input type="number" id="stock" name="stock" value="<?= htmlspecialchars($data['old']['stock'] ?? ''); ?>">
                ...
                <div class="admin-size-grid">
                    <?php foreach ($sizeOptions as $size): ?>
                        <label class="admin-size-pill">
                            <input type="checkbox" name="sizes[]" value="<?= htmlspecialchars($size); ?>" <?= in_array($size, $selectedOldSizes, true) ? 'checked' : ''; ?>>
                ...
                <div class="admin-color-grid">
                    <?php foreach ($data['baseColors'] as $colorName => $hex): ?>
                        <label class="admin-color-pill">
                            <input type="checkbox" name="colors[]" value="<?= htmlspecialchars($colorName); ?>" <?= in_array($colorName, $selectedOldColors, true) ? 'checked' : ''; ?>>
                            <span>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="6"><?= htmlspecialchars($data['old']['description'] ?? ''); ?></textarea>
        ...
        <div class="filter-checkbox">
            <label>
                <input type="checkbox" name="is_featured" value="1" <?= !empty($data['old']['is_featured']) ? 'checked' : ''; ?>>
        ...
        <div class="admin-form-actions">
            ...
            <a href="index.php?url=admin-products" class="btn btn-light">Скасувати</a>
        </div>
    </form>
</section>
